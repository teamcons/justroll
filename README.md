...
To install, use `ninja install`, then execute with `io.github.teamcons.justroll`
...
```bash
ninja io.github.teamcons.justroll-pot
ninja io.github.teamcons.justroll-update-po
```
...
```bash
ninja -C build io.github.teamcons.justroll-update-po
```
...
```bash
flatpak-builder build io.github.teamcons.justroll.yml --user --install --force-clean --install-deps-from=appcenter
```

Then execute with

```bash
flatpak run io.github.teamcons.justroll
```

### Uninstall

```bash
flatpak uninstall io.github.teamcons.justroll --user
```